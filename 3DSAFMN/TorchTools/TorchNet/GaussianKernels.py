...
def random_batch_gaussian_noise_param(im, sig_min=0, sig_max=0.15, mean=0):# 添加随机批量高斯噪声
    """
    ...
    :return:
    """
    B, C, H, W = im.shape# 得到输入图像的批量、通道数、高、宽，
    means = torch.zeros(B, C, H, W)# 生成与im相同形状的全为0的矩阵，均值
    std = torch.ones(B, C, H, W)# 生成与im相同形状的全为1的矩阵，方差
    noise_param = torch.rand(B) * (sig_max - sig_min) + sig_min# 批量噪声
    rand_sig = noise_param.view(B, 1, 1, 1).expand(B, C, H, W)# 对批量维度添加噪声
    sigma = rand_sig * std# 乘上方差
    noise = torch.normal(means, sigma)# 标准化噪声
    return noise, noise_param# 返回噪声和噪声参数
