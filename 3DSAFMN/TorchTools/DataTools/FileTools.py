...
IMG_EXTENSIONS = ['.jpg', '.jpeg', '.png', '.ppm', '.bmp', '.pgm','tif']# 可以读取的所有图像类型
...
def _all_images(path, sort=True):# 获取所有图像文件路径
    """
    ...
    abs_path = os.path.abspath(path)# 获取训练或测试图像文件夹的绝对路径
    image_files = list()# 定义图像文件路径列表
    for subpath in os.listdir(abs_path):# 遍历图像文件夹，得到文件夹下的文件路径
        if os.path.isdir(os.path.join(abs_path, subpath)):# 如果该文件路径是目录
            image_files = image_files + _all_images(os.path.join(abs_path, subpath))# 在迭代一次读取目录下的文件
        else:# 如果该文件路径直接是文件，或者是目录情况下多次迭代后得到文件
            if _is_image_file(subpath):# 如果该文件路径是图像文件
                image_files.append(os.path.join(abs_path, subpath))# 在文件路径中添加该路径
    if sort:# 如果sort为真
        image_files.sort()# 按顺序排序列表
    return image_files# 返回图像文件路径列表
