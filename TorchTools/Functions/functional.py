...
def tensor_block_cat(batch, block=None, overlap=0):
    ...
    real_h = h - 2 * overlap # 真实高度
    real_w = w - 2 * overlap # 真实宽度

    if block is None:# 如果block为空
        root = int(sqrt(N))# 计算root
        if N != root ** 2:# 如果N不等于root的平方
            for i in range(root, N):
                if N % i == 0:
                    root = i
                    break
        block = (int(root), int(N // root))# 得到系数block

    tensor = torch.FloatTensor(B, C, block[0] * real_h, block[1] * real_w)# 得到转换后的tensor
