...
def random_pre_process_pair(hr, lr, lr_patch_size, scale, data_augu):
    """
    ...
    # if mk is not None:
    #     mk_patch = mk.crop((startx * scale, starty * scale,
    #                     (startx + lr_patch_size) * scale, (starty + lr_patch_size) * scale))
    # else:
    #     mk_patch = hr_patch
    ...
    if data_augu:
        if bool(random.getrandbits(1)):
            ...
            # mk_patch = hflip(mk_patch)
            lr_patch = hflip(lr_patch)
        if bool(random.getrandbits(1)):
            hr_patch = vflip(hr_patch)
            # mk_patch = vflip(mk_patch)
            lr_patch = vflip(lr_patch)
        # angle = random.randrange(-15, 15)
        angle = random.randint(0, 3) * 90
        # return rotate(hr_patch, angle), rotate(lr_patch, angle), rotate(mk_patch, angle)
        return rotate(hr_patch, angle), rotate(lr_patch, angle)
    else:
        # return hr_patch, lr_patch, mk_patch
