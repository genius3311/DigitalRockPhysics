...
def GW_loss(x1, x2):# GWloss，CDC的重建损失
    sobel_x = [[-1,0,1],[-2,0,2],[-1,0,1]] # 初始sobel_x
    sobel_y = [[-1,-2,-1],[0,0,0],[1,2,1]] # 初始sobel_y
    b, c, w, h = x1.shape# 得到x1图像b,c,w,h
    sobel_x = torch.FloatTensor(sobel_x).expand(c, 1, 3, 3)# 扩展维度
    sobel_y = torch.FloatTensor(sobel_y).expand(c, 1, 3, 3)# 扩展维度
    sobel_x = sobel_x.type_as(x1)# 使sobel_x和x1类型相同
    sobel_y = sobel_y.type_as(x1)# 使sobel_y和x2类型相同
    weight_x = nn.Parameter(data=sobel_x, requires_grad=False)# 得到权重x
    weight_y = nn.Parameter(data=sobel_y, requires_grad=False)# 得到权重y
    Ix1 = F.conv2d(x1, weight_x, stride=1, padding=1, groups=c)# 得到Ix1
    Ix2 = F.conv2d(x2, weight_x, stride=1, padding=1, groups=c)# 得到IX2
    Iy1 = F.conv2d(x1, weight_y, stride=1, padding=1, groups=c)# 得到Iy1
    Iy2 = F.conv2d(x2, weight_y, stride=1, padding=1, groups=c)# 得到Iy2
    dx = torch.abs(Ix1 - Ix2)# Ix1和Ix2差绝对值
    dy = torch.abs(Iy1 - Iy2)# Iy2和Iy2差绝对值
#     loss = torch.exp(2*(dx + dy)) * torch.abs(x1 - x2)
    loss = (1 + 4*dx) * (1 + 4*dy) * torch.abs(x1 - x2)# 得到损失
    return torch.mean(loss)# 返回损失均值
...
def get_content_loss(loss_type, nn_func=True, use_cuda=False):# 定义内容损失函数
    """
    content loss: [l1, l2, c]# l1,l2和c损失函数
    :param loss_type:# 损失函数类型
    :param nn_func:
    :return:
    TODO: C_loss
    """
    if loss_type == 'l2':# 如果是l2损失函数
        loss = nn.MSELoss() if nn_func else L2_loss # nn_func为真用nn.MSELoss函数，否则用L2_loss
    elif loss_type == 'l1':# 如果是l1损失函数
        loss = nn.L1Loss() if nn_func else L1_loss # nn_func为真用nn.L1Loss函数,否则用L1_loss
    elif loss_type == 'c':# 如果是c损失函数
        loss = C_loss # C_loss
    else:# 其他情况
        loss = nn.MSELoss() if nn_func else L2_loss# nn_func为真用MSELoss函数,否则用L2_loss
    if use_cuda and nn_func:# 如果使用cuda且nn_func为真
        return loss.cuda()# 将loss放到cuda上
    else:# 否则直接返回loss
        return loss
...
class TVLoss(nn.Module):# 降噪损失
    def __init__(self):
        super(TVLoss,self).__init__()

    def forward(self, x):# 前向传播
        batch_size = x.size()[0]# 得到输入图像的批量大小
        h_x = x.size()[2]# 得到输入图像的高
        w_x = x.size()[3]# 得到输入图像的宽
        count_h = self._tensor_size(x[:, :, 1:, :])
        count_w = self._tensor_size(x[:, :, :, 1:])
        h_tv = torch.pow((x[:, :, 1:, :] - x[:, :, :h_x - 1,:]), 2).sum()
        w_tv = torch.pow((x[:, :, :, 1:] - x[:, :, :, :w_x - 1]), 2).sum()
        return 2 * (h_tv / count_h + w_tv / count_w) / batch_size# 返回损失
...
class VGGFeatureExtractor(nn.Module):# VGG特征提取函数
    """
    VGG(
    (features): Sequential(
    (0): Conv2d (3, 64, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1))
    ...
    def __init__(self, vgg_path, layers=34, color_mode='RGB', use_cuda=True, gpus=1):
        ...
        self.color_mode = color_mode# 颜色模式RGB
        vgg = vgg19()# 取vgg19M模型，命名为vgg
        if vgg_path is not '':# 如果vgg_path不为空
            vgg.load_state_dict(torch.load(vgg_path))# 加载vgg_path至vgg中
        self.vgg = nn.Sequential(*list(vgg.features.children())[:(layers + 1)])# 取vgg19前34个符合要求的模块，解包至vgg中
        # layers_dict = [0, 4, 9, 18, 27, 36]
        # layers_dict = [0, 4, 9, 18, 26, 35]
        if gpus > 1:# 如果gpu数量大于1
            self.vgg = nn.DataParallel(self.vgg)# 并行计算
        if use_cuda:# 如果使用cuda
            self.vgg.cuda()# 将vgg模型放到cuda上
        for k, v in self.vgg.named_parameters():# 从vgg.named_parameters()取出k,v
            v.requires_grad = False# v不需要更新梯度

    # def cuda(self, gpus=1):
    #     for Seq in self.vgg:
    #         Seq.cuda()

    def forward(self, input, target):# 前向传播，传入输入图像和目标图像
        # print((np.array(input)).shape)
        # for i in input:
        #     print(i)
        # input= torch.tensor([item.cpu().detach().numpy() for item in input]).cuda()
        # print(input.shape)
        # target= torch.tensor([item.cpu().detach().numpy() for item in target]).cuda()
        if self.color_mode == 'RGB':# 如果颜色模式为RGB
            input_R, input_G, input_B = torch.split(input, 1, dim=1)# 把输入图像划分为R,G,B三个通道
            target_R, target_G, target_B = torch.split(target, 1, dim=1)# 把目标图像划分为R,G,B三个通道
            input_BGR = torch.cat([input_B, input_G, input_R], dim=1)# 将输入图像RGB顺序倒转，重新在dim=1上concat起来
            target_BGR = torch.cat([target_B, target_G, target_R], dim=1)# 将目标图像RGB顺序倒转，重新在dim=1上concat起来
        else:# 如果颜色模式为Y
            input_BGR = input# 直接使用输入图像
            target_BGR = target# 直接使用目标图像

        return self.vgg(input_BGR), self.vgg(target_BGR)# 返回输入图像和目标图像传入vgg网络生成的结果
...
def contextual_Loss(x, y, loss_type='l1', bw_param=1):# 上下文损失函数
    """
    Contextual Loss Base On "The Contextual Loss for Image Transformation" 图像风格迁移产生的损失
    :param x: SR Feature 超分辨率图像特征
    :param y: Target Feature # 目标图像特征
    :param loss_type:# 损失函数类型
    :param bw_param: bandwidth param, 'h' in Paper
    :return:
    """
    def create_dis_mat(x, y, loss_type='l1'):# l1损失函数
        """
        Create Distance Matrix
        """
        B, C, H, W = x.shape# 得到图像的批量、通道、高、宽
        if loss_type == 'l1':# l1损失函数类型
            X = x.contiguous().view(B, C, H * W, 1)# x改变形状，高为H*W,宽为1
            X = X.expand(B, C, H * W, H * W)# X复制扩展
            Y = y.contiguous().view(B, C, H * W, 1)# y改变形状，高为H*W,宽为1
            Y = Y.expand(B, C, H * W, H * W).transpose(2, 3)# Y复制扩展
            D = torch.mean(torch.abs(X - Y), dim=1)# 得到X-Y的绝对值D
        return D # 返回D
